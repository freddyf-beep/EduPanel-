...
import { ArrowLeft, Download, CheckCircle2, AlertCircle, Loader2, List, Send, Printer, Lock, BookOpen, ChevronDown, HardDrive } from "lucide-react"
...
export function ResultadosView({ rubricaId }: Props) {
  ...
  const [subiendoGrupoDrive, setSubiendoGrupoDrive] = useState(false)
  ...
  const handleSubirCompletoDrive = async () => {
    ...
    try {
      ...
      const res = await apiFetch("/api/export-rubrica", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rubrica, evaluacion, profesorNombre, colegio, logoBase64 }),
      })
      if (!res.ok) throw new Error("Error al generar Word")
      const blob = await res.blob()

      const driveContext = {
        tipo: "rubricas" as const,
        asignatura: asignatura || "General",
        curso: rubrica.curso,
      }
      const workspace = await ensureEduPanelWorkspaceForContext(token, driveContext)
      const rubricasFolder = workspace.folders.rubricas
      if (!rubricasFolder) throw new Error("No se pudo crear o encontrar la carpeta en Drive")

      const fileName = `resultados_${rubrica.nombre}_${rubrica.curso}.docx`.replace(/\s+/g, "_")
      await subirDocxYPdfADrive(token, {
        docx: blob,
        folderId: rubricasFolder.id,
        fileName,
      })

      toast({
        title: "Archivo subido a Drive",
        description: `Se subió la rúbrica completa (Word + PDF) a Drive en ${rubricasFolder.name}/.`,
      })
    } catch (e: any) {
      toast({
  ...
  const handleSubirListadoDrive = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Error al generar listado Word")
      const blob = await res.blob()
      ...
      const fileName = `lista_notas_${rubrica.nombre}_${rubrica.curso}.docx`.replace(/\s+/g, "_")
      await subirDocxYPdfADrive(token, {
        docx: blob,
        folderId: rubricasFolder.id,
        fileName,
      ...
      toast({
        ...
        description: `Se subió la lista de notas (Word + PDF) a Drive en ${rubricasFolder.name}/.`,
      })
    } catch (e: any) {
      toast({
        title: "Error al subir a Drive",
  ...
  const handleSubirGrupoDrive = async () => {
    if (!rubrica || !evaluacion) return
    setSubiendoGrupoDrive(true)
    const profesorNombre = auth?.currentUser?.displayName ?? ""
    const colegio = infoColegio?.nombre ?? ""
    const logoBase64 = infoColegio?.logoBase64
    try {
      ...
      if (!res.ok) throw new Error("Error al generar reporte grupal")
      const blob = await res.blob()
      ...
      const fileName = `reporte_grupos_${rubrica.nombre}_${rubrica.curso}.docx`.replace(/\s+/g, "_")
      await subirDocxYPdfADrive(token, {
        docx: blob,
        folderId: rubricasFolder.id,
        fileName,
      ...
      toast({
        title: "Reporte grupal subido a Drive",
        description: `Se subió el reporte por grupos (Word + PDF) a Drive en ${rubricasFolder.name}/.`,
      })
    } catch (e: any) {
      toast({
        title: "Error al subir a Drive",
  ...
  const handleSubirAlumnoDrive = async (estudianteId: string, nombreAlumno: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Error al generar reporte para ${nombreAlumno}`)
      const blob = await res.blob()
      ...
      const cleanAlumnoName = nombreAlumno.replace(/\s+/g, "_")
      const fileName = `rubrica_${cleanAlumnoName}_${rubrica.nombre}.docx`.replace(/\s+/g, "_")
      await subirDocxYPdfADrive(token, {
        docx: blob,
        folderId: rubricasFolder.id,
        fileName,
      ...
      toast({
        ...
        description: `Se subió la rúbrica de ${nombreAlumno} (Word + PDF) a Drive en ${rubricasFolder.name}/.`,
      })
    } catch (e: any) {
      toast({
        title: "Error al subir a Drive",
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              disabled={(exportandoGrupo || exportandoListado || subiendoGrupoDrive || subiendoListadoDrive) || !evaluacion || todosEstudiantes.length === 0}
              className="flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium border border-border rounded-[10px] hover:bg-muted/60 transition-colors disabled:opacity-50"
            >
              {exportandoGrupo || exportandoListado || subiendoGrupoDrive || subiendoListadoDrive ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
          ...
          <DropdownMenuContent align="end" className="w-52">
            ...
            <DropdownMenuItem onClick={handleSubirGrupoDrive} disabled={subiendoGrupoDrive || !evaluacion || todosEstudiantes.length === 0}>
              {subiendoGrupoDrive ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <HardDrive className="w-3.5 h-3.5 text-indigo-500" />}
              Subir grupos (PDF/Word)
            </DropdownMenuItem>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium border border-indigo-200 bg-indigo-50 text-indigo-700 rounded-[10px] hover:bg-indigo-100 transition-colors disabled:opacity-50"
        >
          {subiendoCompletoDrive ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <HardDrive className="w-3.5 h-3.5" />}
                        ...
                        <tr key={est.estudianteId} className="border-b border-border hover:bg-muted/30">
                          ...
                          <td className="px-3 py-2.5 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              <button
                                onClick={() => handleSubirAlumnoDrive(est.estudianteId, est.nombre)}
                                disabled={subiendoAlumnoDrive === est.estudianteId}
                                title={`Subir rúbrica individual de ${est.nombre} a Google Drive`}
                                className="inline-flex items-center justify-center w-7 h-7 rounded-[8px] border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition-colors disabled:opacity-50"
                              >
