...
const ESTADOS: { key: EstadoAsistencia; label: string; cls: string }[] = [
  { key: "presente", label: "P", cls: "bg-green-100 text-green-700" },
  { key: "ausente", label: "A", cls: "bg-red-100 text-red-600" },
  { key: "atraso", label: "T", cls: "bg-amber-100 text-amber-700" },
  { key: "retirado", label: "R", cls: "bg-slate-200 text-slate-700" },
]
...
function buildBloques(curso: string, fecha: string, horarioBase: ClaseHorario[], estudiantes: Estudiante[]): BloqueLibroClase[] {
  ...
  return (bloquesHorario.length ? bloquesHorario : [{ uid: `${curso}-${fecha}-1`, horaInicio: "08:30", horaFin: "09:15", resumen: curso, dia, color: "#F03E6E", tipo: "clase" as const }]).map((bloque, index) => ({
    ...
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button onClick={() => handleGuardar(false)} disabled={saving || saveStatus === "saving_silent"} className="flex items-center gap-2 rounded-[10px] bg-primary text-white px-5 py-2.5 text-[13px] font-bold hover:bg-[#d6335e] disabled:opacity-60">
      ...
      <div className="mb-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[{label:"Presentes", value:resumen.presentes, cls:"bg-green-50 text-green-700"},{label:"Ausentes", value:resumen.ausentes, cls:"bg-red-50 text-red-600"},{label:"Atrasos", value:resumen.atrasos, cls:"bg-amber-50 text-amber-700"},{label:"Retirados", value:resumen.retirados, cls:"bg-slate-100 text-slate-700"}].map((item) => (
            ...
            <div key={bloque.id} className="bg-card border border-border rounded-[16px] overflow-hidden">
              <div className="px-5 py-4 border-b border-border bg-background flex flex-wrap items-center gap-3 justify-between">
                ...
                <div className="flex gap-2 flex-wrap">
                  <button onClick={() => copiarBloqueAnterior(index)} className="rounded-[10px] border border-border px-3 py-2 text-[12px] font-semibold hover:bg-white">Copiar asistencia anterior</button>
              ...
              <div className="grid lg:grid-cols-[1.1fr_1fr] gap-0">
                <div className="p-5 border-b lg:border-b-0 lg:border-r border-border">
                  ...
                  <button onClick={() => setBloques((prev) => prev.map((b) => b.id === bloque.id ? { ...b, firmado: !b.firmado } : b))} className={cn("mt-4 flex items-center gap-2 rounded-[10px] px-4 py-2.5 text-[13px] font-bold", bloque.firmado ? "bg-green-50 text-green-700 border border-green-200" : "bg-card border border-primary text-primary")}>
                    <ShieldCheck className="w-4 h-4" /> {bloque.firmado ? "Firmado internamente" : "Marcar como firmado"}
                  </button>
                </div>

                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3"><UserRound className="w-4 h-4 text-primary" /><h3 className="text-[13px] font-bold">Asistencia</h3></div>
                    ...
                    {bloque.asistencia.map((est) => {
                      ...
                      return (
                        <button key={est.id} onClick={() => toggleAsistencia(bloque.id, est.id)} className="w-full rounded-[12px] border border-border px-3 py-2.5 flex items-center justify-between hover:bg-background transition-colors text-left">
                          <span className="text-[13px] font-medium">{est.nombre}</span>
                          <span className={cn("px-2.5 py-1 rounded-full text-[11px] font-bold", state.cls)}>{state.label}</span>
                        </button>
                      )
                    })}
                  </div>
